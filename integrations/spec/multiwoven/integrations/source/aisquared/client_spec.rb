...
RSpec.describe Multiwoven::Integrations::Source::Aisquared::Client do
  ...
  let(:endpoint) { "http://localhost-bolt:8000/chat" }
  let(:headers) { { "Content-Type" => "application/json" } }
  let(:request_options) { { config: nil } }

  before do
    stub_const(
      "Multiwoven::Integrations::Core::Constants::AISQUARED_BOLT_URL",
      "localhost-bolt:8000"
    )
  ...
  describe "#discover" do
    ...
    it "handles exceptions during discovery" do
      ...
      expect(client).to receive(:handle_exception).with(
        ...
        hash_including(context: "AISQUARED_BOLT:DISCOVER:EXCEPTION", type: "error")
      )
      client.discover
    end
  end
  ...
  describe "#read" do
    ...
    context "when the read operation fails" do
      ...
      it "handles exceptions during reading" do
        ...
        expect(client).to receive(:handle_exception).with(
          error_instance,
          hash_including(context: "AISQUARED_BOLT:READ:EXCEPTION", type: "error")
        )
