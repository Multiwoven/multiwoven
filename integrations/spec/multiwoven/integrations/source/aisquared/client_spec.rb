...
RSpec.describe Multiwoven::Integrations::Source::Aisquared::Client do
  include WebMock::API
  ...
  let(:sync_config_json) do
    {
      source: {
        name: "AisquaredLightningEndpoint",
        type: "source",
      ...
      destination: {
        name: "SampleDestinationConnector",
        type: "destination",
        connection_specification: {
  ...
  let(:endpoint) { "http://localhost-lightning-endpoint:8000/chat" }
  let(:headers) { { "Content-Type" => "application/json" } }
  let(:request_options) { { config: nil } }

  before do
    stub_const(
      "Multiwoven::Integrations::Core::Constants::AISQUARED_LIGHTNING_ENDPOINT_URL",
      "localhost-lightning-endpoint:8000"
    )
  ...
  describe "#discover" do
    ...
    it "handles exceptions during discovery" do
      ...
      expect(client).to receive(:handle_exception).with(
        ...
        hash_including(context: "AISQUARED_LIGHTNING_ENDPOINT:DISCOVER:EXCEPTION", type: "error")
      )
      client.discover
    end
  end
  ...
  describe "#read" do
    ...
    context "when the read operation fails" do
      ...
      it "handles exceptions during reading" do
        ...
        expect(client).to receive(:handle_exception).with(
          error_instance,
          hash_including(context: "AISQUARED_LIGHTNING_ENDPOINT:READ:EXCEPTION", type: "error")
        )
