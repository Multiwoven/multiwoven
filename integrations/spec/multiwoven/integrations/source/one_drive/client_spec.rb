  ...
  describe "#discover" do
    ...
    context "with structured data" do
      ...
      it "uses relative paths as stream names for nested spreadsheets" do
        allow(client).to receive(:fetch_list_items).and_return(
          {
            "value" => [
              {
                "id" => "csv-nested",
                "name" => "sales.csv",
                "relative_path" => "reports/2024/sales.csv"
              }
            ]
          }
        )
        ...
        expect(message.catalog.streams.map(&:name)).to eq(["reports/2024/sales.csv"])
      end
  ...
  describe "#read" do
    context "with structured data" do
      ...
      describe "#extract_file_name_from_query" do
        ...
        it "extracts nested relative paths from queries" do
          expect(client.send(:extract_file_name_from_query, "SELECT * FROM `reports/2024/sales.csv`"))
            .to eq("reports/2024/sales.csv")
        end
      ...
      it "uses the basename when reading a nested spreadsheet path" do
        sync_config[:model][:query] = "SELECT * FROM `reports/2024/sales.csv`"
        s_config = Multiwoven::Integrations::Protocol::SyncConfig.from_json(sync_config.to_json)
        allow(client).to receive(:download_file_to_local).and_return("/tmp/sales.csv")

        expect(client).to receive(:read_local_file).with(
          duckdb_conn,
          "sales.csv",
          "/tmp/sales.csv"
        ).and_return("read_csv_auto('/tmp/sales.csv')")

        records = client.read(s_config)

        expect(records.first.record.data).to eq({ "id" => "1", "amount" => "100" })
        expect(client).to have_received(:download_file_to_local).with("reports/2024/sales.csv", "sync-1")
      end
    end
    ...
    context "with unstructured data" do
      ...
      it "returns relative file_path for nested files from list_files" do
        allow(client).to receive(:fetch_list_items).and_return(
          {
            "value" => [
              {
        ...
        expect(records.first.record.data[:file_name]).to eq("nested.pdf")
        expect(records.first.record.data[:file_path]).to eq("docs/nested.pdf")
      end
      ...
      it "downloads a nested file by relative path" do
        allow(client).to receive(:fetch_list_items).and_return(
          {
            "value" => [
              {
        ...
        unstructured_sync_config[:model][:query] = "download_file docs/nested.pdf"
        s_config = Multiwoven::Integrations::Protocol::SyncConfig.from_json(unstructured_sync_config.to_json)
        allow(client).to receive(:download_file_to_local).and_return("/tmp/nested.pdf")

        records = client.read(s_config)

        expect(records.first.record.data[:file_name]).to eq("nested.pdf")
        expect(records.first.record.data[:file_path]).to eq("docs/nested.pdf")
        expect(client).to have_received(:download_file_to_local).with(
          "nested.pdf",
          "sync-1",
          item_id: "nested-1",
          drive_id: "drive-1"
  ...
  describe "connection and share URL resolution" do
    ...
    describe "#create_connection" do
      context "via user drive" do
        ...
        it "loads is_recursive from the connection config" do
          drive_response = instance_double(
            Net::HTTPSuccess,
            code: "200",
            body: { "id" => "drive-from-user" }.to_json
          )
          allow(Multiwoven::Integrations::Core::HttpClient).to receive(:request).and_return(drive_response)

          client.send(:create_connection, structured_config.merge(is_recursive: true))
          expect(client.instance_variable_get(:@is_recursive)).to be(true)

          client.send(:create_connection, structured_config.merge(is_recursive: "true"))
          expect(client.instance_variable_get(:@is_recursive)).to be(true)

          client.send(:create_connection, structured_config.merge(is_recursive: false))
          expect(client.instance_variable_get(:@is_recursive)).to be(false)
        end
      end
      ...
      context "via share URL" do
        ...
        it "resolves drive_id when share_url is provided without user_name" do
          config_without_user = unstructured_config.dup.tap { |c| c.delete(:user_name) }
          ...
          connection = client.send(:create_connection, config_without_user)

          expect(connection).to be_nil
          expect(client.instance_variable_get(:@drive_id)).to eq("shared-drive-id")
          expect(client.instance_variable_get(:@user_name)).to be_nil
        end
      end

      context "when neither user_name nor share_url is provided" do
        it "raises an error" do
          config_without_user_name = structured_config.dup.tap { |c| c.delete(:user_name) }

          expect do
            client.send(:create_connection, config_without_user_name)
          end.to raise_error(ArgumentError, "User Name or Share URL is required")
        end
      end
    end
  ...
  describe "Graph file operations" do
    ...
    describe "#fetch_list_items" do
      before do
        ...
        client.instance_variable_set(:@share_url, nil)
        client.instance_variable_set(:@data_type, "structured")
        client.instance_variable_set(:@is_recursive, false)
      end
      ...
      it "follows @odata.nextLink and merges all pages" do
        ...
        expect(result["value"].map { |item| item["relative_path"] }).to eq(%w[sales.csv report.xlsx])
      end

      it "skips nested folders when is_recursive is false" do
        root_url = "https://graph.microsoft.com/v1.0/drives/drive-1/root/children"
        ...
        expect(Multiwoven::Integrations::Core::HttpClient).to receive(:request).with(
          ...
          Multiwoven::Integrations::Core::Constants::HTTP_GET,
          headers: graph_auth_headers
        ).once.and_return(root_response)

        result = client.send(:fetch_list_items)

        expect(result["value"].map { |item| item["name"] }).to eq(["sales.csv"])
        expect(result["value"].first["relative_path"]).to eq("sales.csv")
      end

      it "recursively lists nested folder files when is_recursive is true" do
        client.instance_variable_set(:@is_recursive, true)
        root_url = "https://graph.microsoft.com/v1.0/drives/drive-1/root/children"
        nested_url = "https://graph.microsoft.com/v1.0/drives/drive-1/items/folder-1/children"
        ...
        nested_response = instance_double(
          Net::HTTPSuccess,
          code: "200",
          body: {
            "value" => [
              { "id" => "pdf-1", "name" => "notes.pdf" }
            ]
          }.to_json
        )
        ...
        expect(Multiwoven::Integrations::Core::HttpClient).to receive(:request).with(
          ...
          Multiwoven::Integrations::Core::Constants::HTTP_GET,
          headers: graph_auth_headers
        ).ordered.and_return(root_response)

        expect(Multiwoven::Integrations::Core::HttpClient).to receive(:request).with(
          nested_url,
          Multiwoven::Integrations::Core::Constants::HTTP_GET,
          headers: graph_auth_headers
        ).ordered.and_return(nested_response)

        result = client.send(:fetch_list_items)

        expect(result["value"].map { |item| item["name"] }).to eq(%w[sales.csv notes.pdf])
        expect(result["value"].map { |item| item["relative_path"] }).to eq(%w[sales.csv nested/notes.pdf])
      end
    ...
    describe "#single_file_item_url" do
      before do
        client.instance_variable_set(:@drive_id, "drive-1")
        client.instance_variable_set(:@share_url, nil)
      end

      it "preserves path separators for nested file paths" do
        expect(client.send(:single_file_item_url, "reports/2024/sales.csv"))
          .to eq("https://graph.microsoft.com/v1.0/drives/drive-1/root:/reports/2024/sales.csv")
      end

      it "encodes unsafe characters in path segments" do
        expect(client.send(:single_file_item_url, "my reports/q1 sales.csv"))
          .to eq("https://graph.microsoft.com/v1.0/drives/drive-1/root:/my%20reports/q1%20sales.csv")
      end
    end

    describe "#resolve_download_file_name" do
      it "preserves relative paths for non-http inputs" do
        expect(client.send(:resolve_download_file_name, "docs/nested.pdf")).to eq("docs/nested.pdf")
      end

      it "uses configured file_name when the input is an http URL" do
        client.instance_variable_set(:@file_name, "configured.pdf")

        expect(client.send(:resolve_download_file_name, "https://example.com/share/file"))
          .to eq("configured.pdf")
      end
    end

    describe "#matching_file_name?" do
      it "matches by relative path when configured" do
        client.instance_variable_set(:@file_name, "docs/nested.pdf")

        expect(client.send(:matching_file_name?, "nested.pdf", "docs/nested.pdf")).to be(true)
        expect(client.send(:matching_file_name?, "other.pdf", "docs/other.pdf")).to be(false)
      end
    end
  end
