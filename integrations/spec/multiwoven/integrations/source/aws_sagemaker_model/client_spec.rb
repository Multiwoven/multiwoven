...
RSpec.describe Multiwoven::Integrations::Source::AwsSagemakerModel::Client do
  let(:client) { Multiwoven::Integrations::Source::AwsSagemakerModel::Client.new }
  let(:payload) do
    {
      mode: "embedding",
      text_inputs: [
  ...
  describe "#check_connection" do
    ...
    context "when the connection fails" do
      it "returns a failed connection status with an error message" do
        allow_any_instance_of(Multiwoven::Integrations::Source::AwsSagemakerModel::Client).to receive(:create_connection).and_raise(StandardError, "Connection failed")
        message = client.check_connection(sync_config[:source][:connection_specification])
        result = message.connection_status
