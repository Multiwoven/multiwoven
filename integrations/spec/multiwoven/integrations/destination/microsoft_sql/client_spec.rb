...
RSpec.describe Multiwoven::Integrations::Destination::MicrosoftSql::Client do
  let(:client) { Multiwoven::Integrations::Destination::MicrosoftSql::Client.new }
  let(:sync_config) do
    {
      "source": {
  ...
  let(:mssql_result) { instance_double(TinyTds::Result) }
  ...
  describe "#check_connection" do
    context "when the connection is successful" do
      it "returns a succeeded connection status" do
        ...
        allow(mssql_connection).to receive(:active?).and_return(true)
  ...
  describe "#write" do
    context "success" do
      it "write records successfully" do
        ...
        allow(TinyTds::Client).to receive(:new).and_return(mssql_connection)
        allow(mssql_connection).to receive(:active?).and_return(true)
        allow(mssql_connection).to receive(:execute).and_return(mssql_result)
        allow(mssql_result).to receive(:do).and_return(true)
    ...
    context "failure" do
      it "handle record write failures" do
        ...
        allow(mssql_connection).to receive(:execute).and_raise(StandardError.new("test error"))
