...
describe('ConnectorActions', () => {
  ...
  it('deletes source connector successfully', async () => {
    ...
    const deleteButton = screen.getByTestId('confirm-delete-modal-delete-button');
    ...
    expect(mockNavigate).toHaveBeenCalledWith('/setup/sources');
  });
  ...
  it('uses correct connector ID based on type', async () => {
    ...
    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith('/setup/destinations');
    });
  });
});
