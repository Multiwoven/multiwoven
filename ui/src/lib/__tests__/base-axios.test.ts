...
import createBaseAxiosInstance, { _resetLoginRedirecting } from '../base-axios';
...
describe('base-axios', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    // Reset the module-level loginRedirecting singleton so each test starts clean.
    _resetLoginRedirecting();
  ...
  describe('Redirect on 401 - Request Cancellation', () => {
    ...
    it('should not redirect from a second instance once the first instance set the shared flag', () => {
      // Simulate the two-instance scenario: multiwovenFetch (axiosInstance) and
      // enterpriseMultiwovenFetch (enterpriseAxiosInstance) are both created from the same factory.
      // After the module-level loginRedirecting fix, both share one flag.

      // Create a second instance (simulating enterpriseAxiosInstance)
      const instanceB = createBaseAxiosInstance('https://api.example.com/enterprise');
      const responseUseMockB = instanceB.interceptors.response.use as jest.Mock;
      const lastCallIndex = responseUseMockB.mock.calls.length - 1;
      const instanceBErrorHandler = responseUseMockB.mock.calls[lastCallIndex][1] as (
        error: unknown,
      ) => unknown;

      const error = { response: { status: 401 } };

      // First instance triggers the redirect and sets loginRedirecting = true
      capturedResponseErrorHandler!(error);
      expect(window.location.href).toBe('/sign-in');
      expect(mockDeleteCookie).toHaveBeenCalledTimes(1);
      ...
      // Second instance should respect the shared flag and skip the redirect
      instanceBErrorHandler(error);
      expect(window.location.href).toBe('');
      expect(mockDeleteCookie).toHaveBeenCalledTimes(1);
      expect(mockClearState).toHaveBeenCalledTimes(1);
    });
