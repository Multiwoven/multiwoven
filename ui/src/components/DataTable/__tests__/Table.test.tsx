...
describe('DataTable', () => {
  ...
  it('applies dataTestId to the table and tbodyTestId to the tbody', () => {
    render(
      <ChakraProvider>
        <DataTable
          data={data}
          columns={columns}
          dataTestId='orders-datatable'
          tbodyTestId='orders-datatable-tbody'
          getRowProps={(row) => ({ 'data-testid': `row-${row.original.id}` })}
        />
      </ChakraProvider>,
    );

    expect(screen.getByTestId('orders-datatable')).toBeInTheDocument();
    expect(screen.getByTestId('orders-datatable-tbody')).toBeInTheDocument();
    expect(screen.getByTestId('orders-datatable').tagName.toLowerCase()).toBe('table');
    expect(screen.getByTestId('orders-datatable-tbody').tagName.toLowerCase()).toBe('tbody');
  });
});
