...
module EmbedOrigins
  class Registry
    ...
    def self.propagation_meta
      {
        ...
        message: "This change will start reflecting within #{REFRESH_INTERVAL_SECONDS} seconds."
      }
    end
    ...
    class << self
      ...
      def workspace_id_for_agentic_app(agentic_app_id)
        snapshot[:agentic_app][agentic_app_id.to_s]
      end

      def refresh!
        @snapshot = build_snapshot
        Rails.logger.info("[EmbedOrigins::Registry] snapshot set: #{@snapshot.inspect}")
      end
      ...
      def build_snapshot
        ...
        by_workspace.default_proc = nil
        {
          ...
          agentic_app: AgenticCoding::App.where(status: :published).pluck(:id, :workspace_id).to_h.freeze
        }.freeze
      end
