...
module MultiwovenServer
  ...
  class PublishedAppProxy # rubocop:disable Metrics/ClassLength
    ...
    def call(env)
      deployment = resolve_published_app(env)
      return @app.call(env) unless deployment
      return text_response(501, "WebSocket connections are not supported") if websocket_upgrade?(env)

      proxy(env, URI.parse(deployment[:deploy_url]),
            csp_frame_ancestors: frame_ancestors_directive(deployment[:workspace_id]))
    ...
    def resolve_published_app(env)
      ...
      resolve_deployment(label)
    end

    def frame_ancestors_directive(workspace_id)
      return "frame-ancestors 'self'" unless workspace_id
    ...
    def resolve_deployment(label)
      ...
      Rails.application.executor.wrap do
        ...
        row = scope.order(Arel.sql(coalesce)).limit(1).pick(:deploy_url, :workspace_id)
        row && row[0].present? ? { deploy_url: row[0], workspace_id: row[1] } : nil
      end
    end

    def proxy(env, target, csp_frame_ancestors:)
      request = build_request(env, target)
      stream_upstream(target, request, request_host(env), csp_frame_ancestors:)
    end
    ...
    def stream_upstream(target, request, public_host, csp_frame_ancestors:)
      channels = { head: Queue.new, chunks: SizedQueue.new(64) }
      worker = start_upstream_worker(target, request, channels, public_host, csp_frame_ancestors)

      result = channels[:head].pop
      raise "upstream worker exited without a response" if result.nil?
      raise result if result.is_a?(Exception)

      status, headers = result
      [status, headers, StreamingBody.new(channels[:chunks], worker)]
    end

    def start_upstream_worker(target, request, channels, public_host, csp_frame_ancestors)
      head = channels[:head]
      ...
      Thread.new do
        ...
        http.start do |conn|
          conn.request(request) do |response|
            headers = response_headers(response, target.host, public_host)
            inject_csp_frame_ancestors!(headers, csp_frame_ancestors)
            head << [response.code.to_i, headers]
    ...
    def inject_csp_frame_ancestors!(headers, directive)
      return if directive.blank?

      key = "content-security-policy"
      existing = headers[key]
      headers[key] = case existing
                     when nil then directive
                     when Array then existing + [directive]
                     else [existing, directive]
                     end
    end
