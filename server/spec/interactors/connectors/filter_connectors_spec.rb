...
RSpec.describe Connectors::FilterConnectors do
  ...
  describe "#call" do
    ...
    context "with exclude_sub_category filter" do
      let!(:llm_connector) do
        create(:connector, workspace:, connector_type: "source",
                           connector_name: "OpenAI", connector_sub_category: "LLM")
      end
      ...
      it "excludes database connectors when exclude_sub_category is database" do
        result = described_class.call(workspace:, exclude_sub_category: "database")

        expect(result.connectors).to include(llm_connector)
        expect(result.connectors).not_to include(database_connector)
      end

      it "excludes llm connectors when exclude_sub_category is llm" do
        result = described_class.call(workspace:, exclude_sub_category: "llm")

        expect(result.connectors).to include(database_connector)
        expect(result.connectors).not_to include(llm_connector)
      end

      it "returns all connectors when exclude_sub_category is blank" do
        result = described_class.call(workspace:, exclude_sub_category: "")

        expect(result.connectors).to include(llm_connector, database_connector)
      end
    end
