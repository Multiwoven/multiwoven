...
RSpec.describe EmbedOrigins::Registry do
  ...
  describe ".allowed?" do
    ...
    around do |example|
      ...
      ENV["CORS_ALLOWED_ORIGINS"] = "*.squared.ai,localhost,https://internal.example.com"
      example.run
    ensure
    ...
    it "returns true when origin is in the workspace's own allowlist" do
      create(:user_embed_origin,
             created_by_user: user, origin: "https://customer.com",
             organization:, workspace:)
      described_class.refresh!

      expect(described_class.allowed?(workspace.id, "https://customer.com")).to be true
    end

    it "returns true when origin matches a global wildcard from CORS_ALLOWED_ORIGINS" do
      expect(described_class.allowed?(workspace.id, "https://app.squared.ai")).to be true
      expect(described_class.allowed?(workspace.id, "https://staging.squared.ai")).to be true
    end

    it "returns true when origin matches a literal global entry" do
      expect(described_class.allowed?(workspace.id, "https://internal.example.com")).to be true
    end

    it "returns true for localhost patterns from the global list" do
      expect(described_class.allowed?(workspace.id, "http://localhost:3000")).to be true
      expect(described_class.allowed?(workspace.id, "http://127.0.0.1:5173")).to be true
    end

    it "returns false when origin matches neither the workspace nor the global list" do
      expect(described_class.allowed?(workspace.id, "https://evil.com")).to be false
    end

    it "returns false for blank input" do
      expect(described_class.allowed?(workspace.id, nil)).to be false
      expect(described_class.allowed?(workspace.id, "")).to be false
    end

    it "does not leak look-alike subdomains for wildcards" do
      expect(described_class.allowed?(workspace.id, "https://squared.ai.evil.com")).to be false
      expect(described_class.allowed?(workspace.id, "https://evilsquared.ai")).to be false
    end

    context "when CORS_ALLOWED_ORIGINS is the bare '*' sentinel" do
      around do |example|
      ...
      it "returns true for any origin" do
        expect(described_class.allowed?(workspace.id, "https://anything.com")).to be true
      end
    end
  end
end
