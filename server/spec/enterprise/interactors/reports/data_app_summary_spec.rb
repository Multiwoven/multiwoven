...
RSpec.describe Reports::DataAppSummary, type: :interactor do
  ...
  before do
    data_app3.visual_components.order(:id).first.update!(component_type: "chat_bot")
    create(:chat_message, visual_component: visual_component3, role: 0, content: "Hi", session: data_app_session3)
    create(:chat_message, visual_component: visual_component3, role: 1, content: "Hello! How can I help?",
                          session: data_app_session3)
    create(:chat_message, visual_component: visual_component3, role: 0, content: "1+1", session: data_app_session3)
    create(:chat_message, visual_component: visual_component3, role: 1, content: "1+1=2", session: data_app_session3)
  ...
  describe "asset summary counts with custom date ranges" do
    ...
    let!(:data_app_chat) { create(:data_app, workspace:, visual_components_count: 0) }
    ...
    before do
      ...
      chat_session_in_range = create(:data_app_session, workspace:, data_app: data_app_chat)
      ...
      create(:chat_message, visual_component: chat_component, role: 0, content: "Test message",
                            session: chat_session_in_range, workspace:)
      ...
      create(:data_app_session, workspace:, data_app: data_app_visual)
      chat_session_out_of_range = create(:data_app_session, workspace:, data_app: data_app_chat)
      create(:feedback, workspace:, data_app: data_app_visual, visual_component:)
      create(:chat_message, visual_component: chat_component, role: 0, content: "Outside range message",
                            session: chat_session_out_of_range, workspace:)
      create(:message_feedback, visual_component: chat_component, workspace:, data_app: data_app_chat)
    ...
    context "with custom date range filtering" do
      ...
      # TODO: Fix this test
      xit "correctly filters asset counts by custom date range" do
    ...
    context "with custom start_date only (fallback end_date)" do
      ...
      it "includes data within the calculated date range" do
        ...
        # 3 chat messages total (2 in range + 1 outside); total_chat_messages = count / 2
        expect(chat_app_report[:total_chat_messages]).to eq(1)
        # 1 in range + 1 outside
